...
app.post('/HabitatOfAnimal', (req, res) => {
    ...
    db.run('INSERT INTO HabitatOfAnimal (AnimalID, HabitatID) VALUES (?, ?)'
        , habitatOfAnimal.AnimalID, habitatOfAnimal.HabitatID, function (err) {
        if (err) {
            res.status(500).send(err);
        } else {
...
app.put('/HabitatOfAnimal/:id', (req, res) => {
    const habitatOfAnimal = req.body;
    db.run('UPDATE HabitatOfAnimal SET AnimalID = ?, HabitatID = ? WHERE AnimalID = ?', 
            habitatOfAnimal.AnimalID, habitatOfAnimal.HabitatID, req.params.id, function (err) {
        if (err) {
            res.status(500).send(err);
        } else {
            res.send(habitatOfAnimal);
...
app.post('/Animal', (req, res) => {
    ...
    db.run('INSERT INTO Animal (Name, Data, Pic) VALUES (?, ?, ?)', animal.Name, animal.Data, 
                                                                    animal.Pic, function (err) {
        if (err) {
            res.status(500).send(err);
        } else {
...
app.put('/Animal/:id', (req, res) => {
    const animal = req.body;
    db.run('UPDATE Animal SET Name = ?, Data = ?, Pic = ? WHERE ID = ?', animal.Name, animal.Data, 
                                                                        animal.Pic, req.params.id, function (err) {
        if (err) {
            res.status(500).send(err);
        } else {
            res.send(animal);
...
app.post('/Habitat', (req, res) => {
    ...
    db.run('INSERT INTO Habitat (Name, Data, Pic) VALUES (?, ?, ?)', habitat.Name, 
                                                                        habitat.Data, habitat.Pic, function (err) {
        if (err) {
            res.status(500).send(err);
        } else {
...
app.put('/Habitat/:id', (req, res) => {
    const habitat = req.body;
    db.run('UPDATE Habitat SET Name = ?, Data = ?, Pic = ? WHERE ID = ?', habitat.Name, habitat.Data, 
                                                                            habitat.Pic, req.params.id, function (err) {
        if (err) {
            res.status(500).send(err);
        } else {
            res.send(habitat);
